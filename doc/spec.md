...
2. **MODAL 2: 新規タスク登録モーダル & 編集モーダル (`task-modal`)**:
   - **入力項目 (全12項目・スタンドアロン登録画面と完全一致)**:
     1. 旅行タイプ: 「往復」「片道」ボタンスイッチ（片道選択時は復路出発日欄が非表示）。
     2. 出発地: 主要空港の `<datalist id="airportsList">` 補完付きテキスト入力（都市名またはIATA 3レター）。
     3. 目的地: 主要空港の `<datalist id="airportsList">` 補完付きテキスト入力（都市名またはIATA 3レター）。
     4. 往路出発日: カレンダー日付選択（本日以降 `min` ガード）。
     5. 復路出発日: カレンダー日付選択（本日以降 `min` ガード、片道時は非表示）。
     6. 許容乗継回数 (Max Stops): セレクト（「乗継制限なし」「1回乗継まで」「直行便のみ」）。
     7. 巡回間隔: セレクト（全体設定に従う・動的表示 / 3h / 6h / 12h / 24h）。
     8. 目標アラート価格 (JPY): 数値入力。
     9. タスク名 (任意): 監視タスクの識別名（未入力時は `origin ➔ destination` 自動補完）。
     10. 構造化メモ / 要望・制約 (任意): 自然言語・箇条書きテキストエリア。
     11. Webhook通知設定: チェックボックス「デフォルトの Discord / Slack Webhook に通知する」（初期ON。OFF時は個別・全体問わず通知を完全にスキップ）。
     12. ブラウザ表示モード: チェックボックス「定期巡回時もブラウザを表示する (手動支援モード)」（初期OFF）。
   - 送信後: モーダルが自動的に閉じられ、ダッシュボードが即座に最新化（`closeCurrentModal(); showToast('...');`）。
3. **独立画面: スタンドアロン新規タスク登録画面 (`/tasks/new`)**:
   - AI自然言語解析完了後に自動遷移するフルページ登録画面（別タブ展開対応）。
   - **入力項目**: 上記「MODAL 2」と全く同一の全12項目を共通フォーム部品（`render-task-form-fields`）により提供。
   - **エラーハンドリング**: バリデーションエラー発生時は白画面にせず、入力値を完全に保持したままエラーメッセージ付きで再描画する。
   - **ナビゲーション**: キャンセル時、新規タブで開かれている場合はタブを閉じ（`window.close()`）、直接遷移の場合はダッシュボードへ復帰する。
