...
(deftest test-dispatch-price-notification-skips-when-no-webhook
  ...
  (testing "dispatch-price-notification returns {:ok nil} and skips webhook when notification-webhook-url is DISABLED"
    (let [http-client (HttpClient.)
          hnd (:ok (domain/create-iata-code "HND"))
          cdg (:ok (domain/create-iata-code "CDG"))
          now (DateTimeOffset/UtcNow)
          task-item {:id (Guid/NewGuid)
                     ...
                     :target-price-jpy nil
                     :check-interval-hours 12
                     :notification-webhook-url "DISABLED"
                     :user-notes nil
                     :is-headless true
                     :status :active
                     :consecutive-failures 0
          ...
          offer {:id (Guid/NewGuid)
                 ...
                 :booking-url "https://example.com"
                 :captured-at now}
          ;; グローバルWebhookが設定されていても "DISABLED" ならスキップされること
          res (notif/dispatch-price-notification http-client task-item offer -10.0 false "https://discord.com/api/webhooks/dummy")]
      (is (= {:ok nil} res)))))
